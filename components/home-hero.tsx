...
export function HomeHero() {
  ...
  return (
    <section className="relative isolate overflow-hidden bg-[#123d32]">
      <div className="relative min-h-[32rem] sm:min-h-[34rem] lg:min-h-[36rem] xl:min-h-[38rem]">
        <Image
        ...
        <Image
          ...
          placeholder="blur"
          blurDataURL={heroBlurDataUrl}
          className="hidden object-cover object-[32%_center] sm:block"
        />

        <div className="absolute inset-0 bg-gradient-to-r from-white/90 via-white/45 to-transparent sm:from-white/75 sm:via-white/25 lg:w-[58%]" />
        <div className="absolute inset-0 bg-gradient-to-t from-[#123d32]/20 via-transparent to-white/10" />

        <div
          aria-hidden
          className="pointer-events-none absolute -right-[4%] -top-[22%] hidden h-[144%] w-[44%] rounded-[100%] bg-[#123d32] shadow-[-28px_0_60px_rgba(12,40,32,0.16)] lg:block"
        />
        <div
          aria-hidden
          className="pointer-events-none absolute right-0 top-0 hidden h-full w-[22%] bg-gradient-to-l from-[#0e3329] to-transparent lg:block"
        />
        <HeroLeaves />

        <div className="relative z-10 mx-auto flex min-h-[32rem] max-w-7xl flex-col justify-between gap-8 px-4 py-8 sm:min-h-[34rem] sm:px-6 sm:py-10 lg:min-h-[36rem] lg:flex-row lg:items-stretch lg:gap-8 lg:py-11 xl:min-h-[38rem]">
          <div className="flex max-w-3xl flex-1 flex-col justify-between gap-8">
            <div>
              ...
              <nav
                ...
                className="mt-6 grid max-w-lg grid-cols-5 gap-1 sm:mt-7 sm:flex sm:flex-wrap sm:gap-3"
              >
                {heroCategories.map(({ label, href, Icon }) => (
                  <Link
                    key={label}
                    href={href}
                    className="group flex flex-col items-center gap-1.5 sm:gap-1.5 sm:rounded-full sm:bg-white/70 sm:px-3 sm:py-2.5"
                  >
          ...
          <aside className="relative hidden w-[min(20rem,28%)] shrink-0 flex-col items-center justify-center text-center text-white lg:flex">
